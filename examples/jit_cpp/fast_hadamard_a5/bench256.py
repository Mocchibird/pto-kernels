...
def build():
    ...
    subprocess.run([f"{h}/bin/bisheng", "-fPIC", "-shared", "--cce-fatobj-link",
                    "-Wl,-soname,h256.so", str(obj), "-o", str(so)], check=True)
...
def main():
    ...
    print(f"[hadamard256 correctness] b={b}: max_rel={rel:.4g} -> {'OK' if rel < 0.03 else 'FAIL'}")
    ...
    for batch in (16384, 65536):
        ...
        def mk(fn):
            ...
            return run
        cus = time_us(mk(lib.call_copy256)); cg = data / 1e9 / (cus / 1e6)
        for name, fn in (("hadamard", lib.call_hadamard256), ("copy", lib.call_copy256)):
