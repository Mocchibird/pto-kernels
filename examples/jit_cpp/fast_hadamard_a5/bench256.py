...
def build():
    ...
    obj = HERE / "build/h256.o"
    so = HERE / "build/h256.so"
    (HERE / "build").mkdir(exist_ok=True)
    rows = os.environ.get("ROWS", "64")
    nbuf = os.environ.get("NBUF", "4")
    pf = os.environ.get("PF", "2")
...
def main():
    ...
    lib.call_hadamard256(bd, sp(), ctypes.c_void_p(x.data_ptr()), b)
    torch.npu.synchronize()
    out = x.cpu().numpy().astype(np.float32)
    rel = np.abs(out - gold).max() / (np.abs(gold).max() or 1.0)
    print(
        f"[hadamard256 correctness] b={b}: max_rel={rel:.4g} -> {'OK' if rel < 0.03 else 'FAIL'}"
    )
    print(
        "  gold[0,:6]=",
        np.round(gold[0, :6], 2),
    ...
    # timing
    hdr = f"{'batch':>8}  {'kernel':>10}  {'dur_us':>9}  {'GB/s':>8}  {'vs_copy':>8}"
    print("\n" + hdr + "\n" + "-" * len(hdr))
    for batch in (16384, 65536):
        data = 2 * batch * N * 2  # load+store fp16
        POOL = 8
        ...
        cus = time_us(mk(lib.call_copy256))
        cg = data / 1e9 / (cus / 1e6)
        for name, fn in (
            ("hadamard", lib.call_hadamard256),
            ("copy", lib.call_copy256),
        ):
            us = time_us(mk(fn))
            g = data / 1e9 / (us / 1e6)
            print(f"{batch:>8}  {name:>10}  {us:>9.3f}  {g:>8.1f}  {g/cg:>8.3f}")
        print("-" * len(hdr))
