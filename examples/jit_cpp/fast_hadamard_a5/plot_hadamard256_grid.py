...
Reads the CSV emitted by bench256_grid.py (columns rows,nbuf,batch,had_gbs,
...
def _parse_args():
    parser = argparse.ArgumentParser(description="Plot fast_hadamard_256_a5 grid sweep.")
    parser.add_argument("--csv", type=Path, default=DEFAULT_CSV,
                        help=f"Grid CSV from bench256_grid.py (default: {DEFAULT_CSV}).")
    parser.add_argument("--plot-name", type=str, default=DEFAULT_PLOT_NAME,
...
def _draw_heatmap(axis, rows_sorted, batches_sorted, ratio):
    ...
    grid = [[ratio[r].get(b, float("nan")) for b in batches_sorted] for r in rows_bottom_up]
    image = axis.imshow(grid, aspect="auto", origin="lower",
                        cmap="RdYlGn", vmin=0.6, vmax=1.0)
    axis.set_xticks(range(len(batches_sorted)))
    axis.set_xticklabels([f"{b // 1024}k" if b >= 1024 else str(b) for b in batches_sorted],
...
def _draw_bandwidth_line(axis, batches_sorted, had, copy_floor):
    ...
    axis.plot(x, had_tbs, "-", marker="o", color="#2f6df6",
