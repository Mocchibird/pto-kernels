"""The benchmark harness shared by the two fused Hadamard + MXFP4 examples.
...
Each example's ``benchmark.py`` supplies its own loader and width list and
calls :func:`main`; the measurement code is all here.
"""
...
def bench_ladder(build_and_load, k):
...
def bench_copy(build_and_load, k):
...
def main(build_and_load, shapes, description=None):
    ...
    for k in shapes:
        try:
            r = bench_ladder(build_and_load, k)
