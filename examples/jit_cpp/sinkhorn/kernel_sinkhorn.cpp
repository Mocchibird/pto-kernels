...
static_assert(UbOfs::TOTAL <= UB_USABLE_BYTES, "Sinkhorn UB layout exceeds 192 KB.");
...
AICORE void approxLn(uint32_t N, unsigned dataOfs) {
  ...
  TADDS(t1, v, -1.0f);            pipe_barrier(PIPE_V);
  TADDS(v, v, 1.0f);              pipe_barrier(PIPE_V);
  TDIV(r, t1, v);                  pipe_barrier(PIPE_V);
  TMUL(t1, r, r);                 pipe_barrier(PIPE_V);
  TMULS(v, t1, 1.0f / 3.0f);     pipe_barrier(PIPE_V);
  TADDS(v, v, 1.0f);              pipe_barrier(PIPE_V);
  TMUL(v, v, r);                  pipe_barrier(PIPE_V);
  TMULS(v, v, 2.0f);              pipe_barrier(PIPE_V);
}
...
AICORE void runSinkhornImpl(
    __gm__ T *matrix_in, __gm__ T *matrix_out,
  ...
  for (uint32_t bi = wid; bi < N; bi += num_workers) {
    ...
    TEXPANDS(mu1, 1.0f);          pipe_barrier(PIPE_V);
    TEXPANDS(mu2, 1.0f);          pipe_barrier(PIPE_V);
    TEXPANDS(invMu1, 1.0f);       pipe_barrier(PIPE_V);

    {
      uint32_t zLen = Ka > La ? Ka : La;
      Vec1D<float, MAX_DIM> zeroVec(1, zLen);
      TASSIGN(zeroVec, UbOfs::ZERO_VEC);
      TEXPANDS(zeroVec, 0.0f);    pipe_barrier(PIPE_V);
    }
    ...
    for (uint32_t phase = 0; phase <= order; ++phase) {
      ...
      TEXPANDS(colSum,   0.0f);    pipe_barrier(PIPE_V);
      TEXPANDS(colSqsum, 0.0f);    pipe_barrier(PIPE_V);
      ...
      TMUL(scrK, rSum, rSum);          pipe_barrier(PIPE_V);
      TMULS(scrK, scrK, invL);         pipe_barrier(PIPE_V);
      TSUB(rStd, rStd, scrK);          pipe_barrier(PIPE_V);
      TMULS(rStd, rStd, invL1);        pipe_barrier(PIPE_V);
      TMAX(rStd, rStd, zeroK);         pipe_barrier(PIPE_V);
      TSQRT(rStd, rStd);               pipe_barrier(PIPE_V);
      ...
      TMUL(scrL, cSum, cSum);           pipe_barrier(PIPE_V);
      TMULS(scrL, scrL, invK);          pipe_barrier(PIPE_V);
      TSUB(cStd, cStd, scrL);           pipe_barrier(PIPE_V);
      TMULS(cStd, cStd, invK1);         pipe_barrier(PIPE_V);
      TMAX(cStd, cStd, zeroL);          pipe_barrier(PIPE_V);
      TSQRT(cStd, cStd);                pipe_barrier(PIPE_V);
      ...
      if (phase == 0) {
        ...
        TMIN(sA, sA, sB);              pipe_barrier(PIPE_V);
        TADDS(sA, sA, eps);            pipe_barrier(PIPE_V);
      } else {
        // ---- mu update ----
        ...
        TROWEXPANDDIV(rStdUpd, rStdUpd, tgtCol);   pipe_barrier(PIPE_V);
        {
          Vec1D<float, MAX_DIM> epsVec(1, Ka);
          TASSIGN(epsVec, UbOfs::LN_TMP1);
          TEXPANDS(epsVec, 1e-12f);                 pipe_barrier(PIPE_V);
          TMAX(rStdUpd, rStdUpd, epsVec);           pipe_barrier(PIPE_V);
        }
        approxLn<MAX_DIM>(Ka, UbOfs::ROW_SQSUM);
        TASSIGN(rStdUpd, UbOfs::ROW_SQSUM);
        TMULS(rStdUpd, rStdUpd, lr);                pipe_barrier(PIPE_V);
        TEXP(rStdUpd, rStdUpd);                     pipe_barrier(PIPE_V);
        Vec1D<float, MAX_DIM> mu2Upd(1, Ka);
        TASSIGN(mu2Upd, UbOfs::MU2);
        TMUL(mu2Upd, mu2Upd, rStdUpd);             pipe_barrier(PIPE_V);

        // mu1 *= pow(col_std / tgt, lr)
        Vec1D<float, MAX_DIM> cStdUpd(1, La);
        TASSIGN(cStdUpd, UbOfs::COL_SQSUM);
        TROWEXPANDDIV(cStdUpd, cStdUpd, tgtCol);   pipe_barrier(PIPE_V);
        {
          Vec1D<float, MAX_DIM> epsVec(1, La);
          TASSIGN(epsVec, UbOfs::LN_TMP1);
          TEXPANDS(epsVec, 1e-12f);                 pipe_barrier(PIPE_V);
          TMAX(cStdUpd, cStdUpd, epsVec);           pipe_barrier(PIPE_V);
        }
        approxLn<MAX_DIM>(La, UbOfs::COL_SQSUM);
        TASSIGN(cStdUpd, UbOfs::COL_SQSUM);
        TMULS(cStdUpd, cStdUpd, lr);                pipe_barrier(PIPE_V);
        TEXP(cStdUpd, cStdUpd);                     pipe_barrier(PIPE_V);
        Vec1D<float, MAX_DIM> mu1Upd(1, La);
        TASSIGN(mu1Upd, UbOfs::MU1);
        TMUL(mu1Upd, mu1Upd, cStdUpd);             pipe_barrier(PIPE_V);

        // Refresh inv_mu1 and re-tile
        Vec1D<float, MAX_DIM> ones(1, La);
        TASSIGN(ones, UbOfs::LN_TMP1);
        TEXPANDS(ones, 1.0f);                       pipe_barrier(PIPE_V);
        Vec1D<float, MAX_DIM> newInv(1, La);
        TASSIGN(newInv, UbOfs::INV_MU1);
        TASSIGN(mu1Upd, UbOfs::MU1);
        TDIV(newInv, ones, mu1Upd);                 pipe_barrier(PIPE_V);
        tileInvMu1<TileL>(La);
      }
    }  // phase loop
...
AICORE void runSinkhorn(__gm__ T *matrix_in, __gm__ T *matrix_out,
                        ...
                        float eps, float invK, float invL,
                        float invK1, float invL1) {
  set_mask_norm();
  set_vector_mask(-1, -1);
  ...
  switch (La) {
    case 64:
      runSinkhornImpl<T, 64>(matrix_in, matrix_out, mu1_out, mu2_out,
                             N, K, L, La, Ka, order, lr, eps, invK, invL, invK1, invL1);
      break;
    case 128:
      runSinkhornImpl<T, 128>(matrix_in, matrix_out, mu1_out, mu2_out,
                              N, K, L, La, Ka, order, lr, eps, invK, invL, invK1, invL1);
      break;
    default:
      // La <= 32 or La >= 192: use MAX_DIM stride (long flat vectors)
      runSinkhornImpl<T, MAX_DIM>(matrix_in, matrix_out, mu1_out, mu2_out,
                                  N, K, L, La, Ka, order, lr, eps, invK, invL, invK1, invL1);
      break;
  }
}
...
extern "C" __global__ AICORE void sinkhorn_fp16(GM_ADDR matrix_in,
                                                GM_ADDR matrix_out,
                                                GM_ADDR mu1_out,
                                                GM_ADDR mu2_out, uint32_t N,
                                                uint32_t K, uint32_t L,
                                                uint32_t order, float lr,
                                                float eps, float invK,
                                                float invL, float invK1,
                                                float invL1) {
#if __CCE_AICORE__ == 220 && defined(__DAV_C220_VEC__)
...
#else
  (void)matrix_in; (void)matrix_out; (void)mu1_out; (void)mu2_out;
  (void)N; (void)K; (void)L; (void)order; (void)lr; (void)eps;
  (void)invK; (void)invL; (void)invK1; (void)invL1;
#endif
}
...
extern "C" void call_sinkhorn_kernel(uint32_t blockDim, void *stream,
                                     ...
                                     float invL1) {
  sinkhorn_fp16<<<blockDim * 2, nullptr, stream>>>(matrix_in, matrix_out,
                                                   mu1_out, mu2_out, N, K, L,
                                                   order, lr, eps, invK, invL,
                                                   invK1, invL1);
}
