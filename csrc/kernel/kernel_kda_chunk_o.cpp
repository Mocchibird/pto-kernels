...
// Resolve a global chunk index to the sequence that owns it: yields that
// sequence's token base and length, plus the number of chunks in all the
// sequences before it.  The varlen scan is over num_seqs scalars, which is
// negligible next to a chunk body.
AICORE inline void ResolveGlobalChunk(__gm__ int32_t* cu_seqlens,
                                      int64_t num_seqs, int64_t seq_len,
                                      int32_t chunk_size, int64_t gci,
                                      int64_t& bos, int64_t& slen,
  ...
  if (cu_seqlens != nullptr) {
    for (int64_t si = 0; si < num_seqs; ++si) {
      ...
      const int64_t nc = (slen + chunk_size - 1) / chunk_size;
      if (gci - chunk_offset < nc) break;
      chunk_offset += nc;
    }
  } else {
    const int64_t chunks_per_seq = (seq_len + chunk_size - 1) / chunk_size;
    const int64_t seq_idx = gci / chunks_per_seq;
    bos = seq_idx * seq_len;
    slen = seq_len;
    chunk_offset = seq_idx * chunks_per_seq;
  }
}
...
AICORE void kda_chunk_o_kernel(__gm__ half* Q_handle, __gm__ half* K_handle,
  ...
  // Work items are (chunk, head).  Chunks within a (seq, head) are fully
  // independent -- each reads its own s_snapshots entry -- so enumerating
  // them flatly gives num_chunks times more items and fills the device
  // instead of leaving all but num_seqs*H cores idle.
  int64_t total_chunks = 0;
  if (cu_seqlens != nullptr) {
    for (int64_t si = 0; si < num_seqs; ++si) {
      const int64_t sb = static_cast<int64_t>(cu_seqlens[si]);
      const int64_t se = static_cast<int64_t>(cu_seqlens[si + 1]);
      total_chunks += (se - sb + C - 1) / C;
    }
  } else {
    total_chunks = num_seqs * ((seq_len + C - 1) / C);
  }
  ...
  for (int64_t pid = cid; pid < total_work; pid += block_num) {
    int64_t head = pid % H;
    int64_t gci = pid / H;  // global chunk index

    int64_t bos = 0, slen = 0, chunk_offset = 0;
    ResolveGlobalChunk(cu_seqlens, num_seqs, seq_len, C, gci, bos, slen,
                       chunk_offset);
    int32_t ci = static_cast<int32_t>(gci - chunk_offset);
